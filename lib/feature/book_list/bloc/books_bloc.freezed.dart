...
abstract class $BooksStateCopyWith<$Res> {
  ...
  $Res call({List<BookModel> books, bool isLoading});
}
...
class _$_BooksState implements _BooksState {
  const _$_BooksState(
      {final List<BookModel> books = const [], this.isLoading = false})
      : _books = books;
  ...
  @override
  String toString() {
    return 'BooksState(books: $books, isLoading: $isLoading)';
  }
...
abstract class _BooksState implements BooksState {
  const factory _BooksState(
      {final List<BookModel> books, final bool isLoading}) = _$_BooksState;
